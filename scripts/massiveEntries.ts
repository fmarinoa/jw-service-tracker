...
function mapSessionType(input: string): SessionType {
    ...
    if (normalized.includes("casa") || normalized.includes("house") || normalized.includes("publica")) {
        return "house_to_house";
    }
    if (normalized.includes("revisita") || normalized.includes("revisit")) {
        return "revisits";
    }
    if (normalized.includes("estudio") || normalized.includes("study") || normalized.includes("biblico") || normalized.includes("bible")) {
        return "bible_study";
    }
    ...
    if (["house_to_house", "revisits", "bible_study", "other"].includes(normalized)) {
...
async function main() {
    ...
    if (args.length === 0) {
        console.error("\x1b[31mError: Debes proporcionar la ruta al archivo CSV.\x1b[0m");
    ...
    if (!mongoUri) {
        console.error('\x1b[31mError: La variable de entorno "MONGODB_URI" no está configurada.\x1b[0m');
        process.exit(1);
    }
    ...
    if (lines.length === 0) {
        console.error("\x1b[31mError: El archivo CSV está vacío o solo contiene líneas vacías.\x1b[0m");
        await client.close();
        process.exit(1);
    }
    ...
    for (let lineNum = 2; lineNum <= lines.length; lineNum++) {
        ...
        try {
            ...
            if (!ObjectId.isValid(userId)) {
                throw new Error(`customerId "${userId}" tiene un formato de ObjectId de MongoDB inválido`);
            }
            ...
            if (preachingDate > DateTime.now().toMillis()) {
                throw new Error(`La fecha de predicación (${rawFecha}) no puede ser futura`);
            }

            // Parse hours & minutes
            const hours = parseInt(rawHoras || "0", 10);
            const minutes = parseInt(rawMinutos || "0", 10);
            if (isNaN(hours) || hours < 0) throw new Error(`Horas inválidas: "${rawHoras}"`);
            if (isNaN(minutes) || minutes < 0 || minutes > 59) throw new Error(`Minutos inválidos: "${rawMinutos}"`);

            const totalMinutes = (hours * 60) + minutes;
            if (totalMinutes === 0) {
                throw new Error("El tiempo total de la sesión debe ser mayor a 0 (0 horas y 0 minutos no permitido)");
            }
            ...
            if (notes && notes.length > maxLength) {
                console.warn(`\x1b[33mFila ${lineNum}: Nota recortada a ${maxLength} caracteres (original: ${notes.length} caracteres)\x1b[0m`);
            ...
            console.log(`\x1b[32m✓ Fila ${lineNum}: Entrada creada para usuario ${user.name} (${hours}h ${minutes}m - ${type})\x1b[0m`);
            successCount++;
        } catch (error: any) {
            console.error(`\x1b[31m✗ Fila ${lineNum} (Error): ${error.message}\x1b[0m`);
            failCount++;
        }
    }
