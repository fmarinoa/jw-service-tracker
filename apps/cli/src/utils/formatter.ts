...
import { UserDoc, EntryDoc } from '../services';
...
export class OutputFormatter {
  ...
  static printUserHeader(user: UserDoc): void {
    ...
    console.log(`🎯 Meta:       ${user.monthlyGoal || 0} horas (${user.preacherType || 'publicador'})\n`);
  }
  ...
  static printEntriesTable(entries: EntryDoc[]): void {
    console.log(pc.bold(pc.underline('Historial de Entradas (Más recientes primero):')));
    ...
    console.log(pc.gray('-'.repeat(colWidths.date + colWidths.time + colWidths.type + colWidths.notes + 9)));

    let totalMinutes = 0;

    entries.forEach((entry) => {
      const date = DateTime.fromMillis(entry.preachingDate).toFormat('yyyy-MM-dd');
      const time = `${entry.hours}h ${entry.minutes}m`;
      const typeName = SESSION_TYPE_NAMES[entry.type] || entry.type;
      const notes = entry.notes ? (entry.notes.length > 27 ? entry.notes.slice(0, 27) + '...' : entry.notes) : '-';
  ...
  static printMonthlySummary(entries: EntryDoc[]): void {
    const monthlyStats: Record<string, { totalMin: number; byType: Record<string, number> }> = {};

    entries.forEach((entry) => {
      const duration = (entry.hours || 0) * 60 + (entry.minutes || 0);
      const monthKey = DateTime.fromMillis(entry.preachingDate).toFormat('yyyy-MM');
      
      if (!monthlyStats[monthKey]) {
        monthlyStats[monthKey] = { totalMin: 0, byType: {} };
      }
      monthlyStats[monthKey].totalMin += duration;
      monthlyStats[monthKey].byType[entry.type] = (monthlyStats[monthKey].byType[entry.type] || 0) + duration;
    });
    ...
    Object.keys(monthlyStats).sort().reverse().forEach((month) => {
      ...
      console.log(`\n📅 ${pc.bold(month)}`);
      console.log(`  ⏱️ Tiempo Real: ${pc.yellow(`${actualHrs}h ${actualMins}m`)}`);
      console.log(`  📋 Horas Reportables: ${pc.green(`${reportableHrs} horas`)} (redondeado para informe)`);
    });
    console.log();
  }
